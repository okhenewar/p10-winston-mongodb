...
//changed insertOne code to support insertMany using throttle.
let 
entries = [],
updateCollection = (thisInst, cb, next) => {

  let cuhnkedArrayOf100s = _.chunk(entries,100)
  entries = [];
  
  _.forEach(cuhnkedArrayOf100s, (arrof100, i) => {
    thisInst.logDb.collection(thisInst.collection).insertMany(arrof100).then((data)=>{
      thisInst.emit('logged');
      console.log('logged to mongo.');
      //next(null, true);
    }).catch(err=>{
      thisInst.emit('error', err);
      console.error('error while logging into mongodb.', err);
      //error(err);
    });
  })
...
throttledUpdateFun = _.throttle(updateCollection,5000,{trailing:true,leading:false});
 ...
 */
MongoDB.prototype.log = function(level, message, info, cb) {
  if (!this.logDb) {
  ...
  process.nextTick(()=>{
    ...
    let entry = {timestamp: new Date(), level: level};

    //let message = util.format(info[MESSAGE], ...(info.splat || []));
    message = util.format(message, ...(info.splat || []));
    ...
    entries.push(entry);
    throttledUpdateFun(this);
    cb();

    /* //previous code where insertOne is used.
